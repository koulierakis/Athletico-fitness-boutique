(()=>{
  ...
  if(currentScript?.src){
    [
      ...
      ['award-direction','../css/award-direction-v1.css?v=20260907-2'],
      ['award-experience','../css/award-experience-v1.css?v=20260907-2']
    ].forEach(([key,path])=>{
  ...
  const readCMS=()=>{try{return JSON.parse(localStorage.getItem(CMS_KEY)||localStorage.getItem(legacyKey)||'{}')}catch{return {}}};
  const cms=readCMS();
  ...
  const applyImg=(node,key)=>{
    ...
    if(e){
      ...
      if(e.shadow!==undefined)node.style.boxShadow=`0 24px ${Math.max(20,e.shadow)}px rgba(0,0,0,${Math.min(.7,(e.shadow||0)/100)})`;
    }
  };
  document.querySelectorAll('[data-image-key]').forEach(node=>applyImg(node,node.dataset.imageKey));
  document.querySelectorAll('[data-image-bg-key]').forEach(node=>{
    const value=images[node.dataset.imageBgKey];
    if(value)node.style.backgroundImage=`url("${value.replace(/"/g,'&quot;')}")`;
  });
  ...
  Object.entries(generic).forEach(([selector,o])=>{
    ...
    if(o.style)Object.entries(o.style).forEach(([prop,val])=>{
      const cssProp=prop.replace(/[A-Z]/g,m=>'-'+m.toLowerCase());
      node.style.setProperty(cssProp,val,'important');
    });
  });
  ...
  if(nav&&!nav.querySelector('.admin-nav-link')){
    ...
    adminLink.style.marginTop='14px';
    adminLink.style.opacity='.62';
    const strong=adminLink.querySelector('strong');
    if(strong){strong.style.fontSize='.72em';strong.style.fontWeight='400'}
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(e.key!=='Tab')return;
    const items=[...overlay.querySelectorAll(focusable)].filter(item=>!item.hasAttribute('hidden'));
  ...
  const isHome=/\/(index\.html)?$/.test(location.pathname)||location.pathname.endsWith('/work/athletico-awards-rebuild-v2/');
  const heroKicker=document.querySelector('.luxury-hero-kicker');
  if(heroKicker&&heroKicker.textContent.trim()==='MORE THAN A GYM')heroKicker.textContent='PERSONAL WELLNESS · SINCE 2000';

  const philosophyTitle=document.querySelector('#philosophy h2');
  if(philosophyTitle&&!content.philosophyTitle)philosophyTitle.textContent='Η ευεξία δεν είναι μία στιγμή.';
  const philosophyLead=document.querySelector('#philosophy .lead');
  if(philosophyLead&&!content.philosophyLead)philosophyLead.textContent='Είναι ο τρόπος που επιλέγεις να φροντίζεις τον εαυτό σου.';
  const philosophyText=document.querySelector('#philosophy .muted');
  if(philosophyText&&!content.philosophyText)philosophyText.textContent='Προσωπική παρουσία, ουσιαστική καθοδήγηση και κίνηση που αποκτά θέση στην καθημερινότητά σου.';
  ...
  if(isHome){
    ...
    if(teamSection&&!teamSection.querySelector('.home-team-editorial')){
      ...
      if(grid){
        grid.className='home-team-editorial';
        grid.innerHTML=`<a class="home-team-person" href="team.html"><img src="assets/images/giannis-profile-new2.png?v=20260906f" alt="Γιάννης Κουλιεράκης"><div class="home-team-copy"><span class="home-team-index">01 · PERSONAL PRESENCE</span><h3>Γιάννης Κουλιεράκης</h3><p>Πτυχιούχος ΤΕΦΑΑ · Pilates Reformer Instructor</p><span class="btn ghost">ΓΝΩΡΙΣΕ ΤΗΝ ΟΜΑΔΑ</span></div></a><a class="home-team-person" href="team.html"><img src="assets/images/sofia-profile-new.png?v=20260906f" alt="Σοφία Πολύζου"><div class="home-team-copy"><span class="home-team-index">02 · PERSONAL PRESENCE</span><h3>Σοφία Πολύζου</h3><p>Πτυχιούχος ΤΕΦΑΑ · Personal & Mini Group Instructor</p><span class="btn ghost">ΓΝΩΡΙΣΕ ΤΗΝ ΟΜΑΔΑ</span></div></a>`;
      }
    }
  }
  ...
  if(!document.querySelector('.award-side-mark')){
    ...
    mark.className='award-side-mark';
    mark.textContent='ATHLETICO · PERSONAL WELLNESS · TRIKALA';
    document.body.appendChild(mark);
  }

  if(!document.querySelector('.award-page-transition')){
    const transition=document.createElement('div');
    transition.className='award-page-transition';
    transition.setAttribute('aria-hidden','true');
    document.body.appendChild(transition);
    document.querySelectorAll('a[href]').forEach(a=>{
      const href=a.getAttribute('href')||'';
      if(!href||href.startsWith('#')||href.startsWith('mailto:')||href.startsWith('tel:')||a.target==='_blank'||href.startsWith('http'))return;
